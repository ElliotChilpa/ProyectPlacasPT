...
public class TarjetaRFIDDAO {
    ...
    /**
     * Devuelve el ID_Usuario_Fijo asociado a un tag RFID activo,
     * o -1 si no existe o no está activo.
     */
    public int obtenerIdUsuarioFijoPorTag(String tag) {
        String sql = """
            SELECT FK_Usuario_Fijo
              FROM TarjetaRFID
             WHERE ID_RFID = ?
               AND Estado_Vigencia = 1
             LIMIT 1
        """;
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return rs.getInt("FK_Usuario_Fijo");
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
